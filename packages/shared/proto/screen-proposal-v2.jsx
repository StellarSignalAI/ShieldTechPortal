      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24, background: 'rgba(5,7,10,0.3)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button onClick={() => { if (window.__shieldPdf) window.__shieldPdf.exportDoc({ kind: 'proposal', number: (proposal && proposal.id) || 'Proposal', customer: (proposal && proposal.customer) || '', date: new Date().toLocaleDateString(), sections: (proposal && proposal.sections) ? proposal.sections.map(x => ({ title: x.title || x.name || '', body: x.body || x.text || '' })) : [{ title: 'Scope', body: (proposal && proposal.scope) || 'Full scope of work.' }], lineItems: (proposal && proposal.lineItems) || [], total: (proposal && proposal.total) || undefined }); else shieldToast('Export unavailable'); }} style={{ padding: '5px 14px', background: 'rgba(63,169,245,0.06)', border: '1px solid var(--border-subtle)', borderRadius: 6, color: 'var(--brand)', fontSize: 11, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>Download PDF</button>
