...
// Auth: Bearer RIPPLING_API_TOKEN. Optional Rippling-API-Version header from
// RIPPLING_API_VERSION. Cursor pagination (limit/cursor → next_cursor).
//
...
export function ripplingConfigured(): boolean {
  return Boolean(Deno.env.get("RIPPLING_API_TOKEN"));
}
...
export async function ripplingRequest(
  ...
  if (!token) throw new RipplingError("Rippling not configured (RIPPLING_API_TOKEN missing)", 503, cid);
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (res.ok) {
        console.log(JSON.stringify({ evt: "rippling.request", cid, path: path.split("?")[0], status: res.status, attempt }));
        return body;
      }
  ...
  console.log(JSON.stringify({ evt: "rippling.request", cid, path: path.split("?")[0], status: lastErr?.status ?? 0, error: true }));
...
export async function* ripplingPaginate(
  path: string,
  { limit = 100, maxPages = 50, correlationId }: { limit?: number; maxPages?: number; correlationId?: string } = {},
): AsyncGenerator<Record<string, unknown>> {
  let cursor: string | null = null;
  for (let page = 0; page < maxPages; page++) {
    const sep = path.includes("?") ? "&" : "?";
    const url = `${path}${sep}limit=${limit}${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`;
    const body = (await ripplingRequest(url, { correlationId })) as Record<string, unknown>;
